...
| **B10 Performance report** | `GET /report?period=week\|month` returns the CORE.md §7 shape; trend/consistency math hand-verified against the real seeded Eyang Uti data (not just spot-checked) for both week and month windows; zero-data elder gets a gentle 200 with `medication_adherence_pct: null`, not a fabricated number or an error. Copy is English per CORE.md's literal example, not Indonesian like the rest of the product — flagged for a second look before demo, not silently shipped. First cut if Day 3 runs short. |
